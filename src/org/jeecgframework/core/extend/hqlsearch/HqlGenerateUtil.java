...
public class HqlGenerateUtil {
	...
	private static void installHqlJoinAlias(CriteriaQuery cq, Object searchObj,
		...
		for (int i = 0; i < origDescriptors.length; i++) {
			...
			boolean flag = origDescriptors[i].getReadMethod().isAnnotationPresent(Transient.class) ;
			
			type = origDescriptors[i].getPropertyType().toString();
			try {
				if (judgedIsUselessField(name) || flag
	...
	/*private static boolean judgedIsTransientFiled( PropertyDescriptor origDescriptor)
	{
		return origDescriptor.getReadMethod().isAnnotationPresent(Transient.class) ;
	}*/
