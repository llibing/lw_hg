...
public class UserController {
	...
	public AjaxJson del(TSUser user, HttpServletRequest req) {
		...
		List<TSLog> tsLog = systemService.findByProperty(TSLog.class, "TSUser.id", user.getId());
	...
	private void delLog(TSUser user) {
		// TODO Auto-generated method stub
		 //同步删除用户角色关联的日志表
		List<TSLog> tsLogList = systemService.findByProperty(TSLog.class, "TSUser.id", user.getId());
		if (tsLogList.size() >= 1) {
